...
const oldPath = __dirname + '/pdf-txt-copia/fernandohenriquecardoso/';
...
//         fs.rename(fromPath, `${oldPath}${Math.random(index)}_fhc.txt`, (err) => {
...
(async () => {
  try {
    ...
    for (const [index, file] of data.entries()) {
      ...
      fs.rename(fromPath, `${oldPath}fernandoHenriqueCardoso_${index}.txt`, (err) => {
